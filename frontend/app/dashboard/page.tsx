...
export default function DashboardPage() {
  ...
  <form onSubmit={handleAddResource} className="space-y-5">
    ...
    {/* GPS Location — the important part */}
    <div>
      <label className="block text-xs font-medium text-gray-700 mb-2">
        Depot GPS Location <span className="text-red-500">*</span>
      </label>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-dashed border-green-300 text-green-600 text-sm font-medium hover:bg-green-50 hover:border-green-400 transition mb-3"
      >
      ...
      {resForm.depot_lat && resForm.depot_lng ? (
        <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-4 py-3 mb-3">
          ...
          <button
            type="button"
            onClick={() => setResForm({ ...resForm, depot_lat: "" as any, depot_lng: "" as any })}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-green-500 font-mono"
            placeholder="22.5726"
            required
          />
        </div>
      ...
      <p className="text-xs text-gray-400 mt-2">
        💡 Tip — To find coordinates of any location: open Google Maps → right-click the location → the first line shows lat, lng
      </p>
    </div>

    <button
      type="submit"
      className="w-full bg-green-600 text-white py-3.5 rounded-xl text-sm font-semibold hover:bg-green-700 active:scale-95 transition"
    >
